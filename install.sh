...
# Prompt user for MongoDB setup details
container_name=$(prompt_user "Enter the container name" "mongodb")
network_name=$(prompt_user "Enter the network name" "general")
mongodb_port=$(prompt_user "Enter the MongoDB port" "27017")
mongodb_user=$(prompt_user "Enter the MongoDB username" "admin")
mongodb_password=$(prompt_user "Enter the MongoDB password" "admin123")
allow_host=$(prompt_user "Enter the allowed host" "0.0.0.0")

# Generate the .env file
echo "Creating .env file for MongoDB setup..."
cat > .env <<EOL
...
echo "Starting Docker Compose with --build for MongoDB..."
if docker-compose up -d --build; then
    echo "Checking container status..."
    if [ "$(docker inspect -f '{{.State.Running}}' $container_name)" = "true" ]; then
        echo "MongoDB setup is complete and running on port $mongodb_port."
    else
